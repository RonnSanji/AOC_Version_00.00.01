...
- (id) init {
    ...
    if (self) {
        ...
        // 放一个背景图片精灵
        CCSprite *sp = [CCSprite spriteWithFile:@"level.png"];
...
//        }
          CCSprite *levelSprite1 = [CCSprite spriteWithFile:@"stage1.png"];
         levelSprite1.tag = 1;
          levelSprite1.position=ccp(winSize.width/2.0f, winSize.height/2.0f);
        
        [self addChild:levelSprite1];
    }
    return self;
}
...
- (void) ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (int i = 0; i < self.children.count; i++) {
        ...
        if (CGRectContainsPoint(oneSprite.boundingBox, nodePoint) && oneSprite.tag == 100) {
            ...
            [trans release];
        } else if (CGRectContainsPoint(oneSprite.boundingBox, nodePoint) && (oneSprite.tag < 100) && oneSprite.tag >0) {
            NSLog(@"选中了第 %d 关", oneSprite.tag);
            CCScene *sc = [GamePlay scene];
